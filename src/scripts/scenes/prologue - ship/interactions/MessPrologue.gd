...
func look_pressed(look) -> void:
	_play_story(look, "prologue_ren.mess_look")


func use_pressed(use) -> void:
	_play_story(use, "prologue_ren.mess_use")


func item_pressed(item) -> void:
	if item.text == "Scanner":
		if StoryRunner.story.variables_state.get("mess_scanned") == 0:
			_play_story(item, "prologue_ren.mess_scan")
		else:
